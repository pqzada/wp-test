...
class validation{
    ...
    */
     ...
     *
     */
    public function addRule($varname, $label, $type, $required=false, $min=0, $max=0, $trim=false)
    {
        $this->validation_rules[$varname] = array('label', $label, 'type'=>$type, 'required'=>$required, 'min'=>$min, 'max'=>$max, 'trim'=>$trim);
        /*** allow chaining ***/
        return $this;
    }
    ...
    {
        if(!isset($this->source[$var]) || strlen($this->source[$var]) == 0)
        {
            $this->errors[$var] = $this->validation_rules[$var]['label'] . __(' No ha sido ingresado', 'validate_anaconda');
        }
    }
    ...
    private function validateIpv4($var, $required=false)
    {
        ...
        {
            $this->errors[$var] = $this->validation_rules[$var]['label'] . __(' No es una IP válida', 'validate_anaconda');
        }
    }
    ...
    private function validateFloat($var, $required=false)
    {
        ...
        {
            $this->errors[$var] = $this->validation_rules[$var]['label'] . __(' No es un decimal válido', 'validate_anaconda');
        }
    }
    ...
    private function validateString($var, $min=0, $max=0, $required=false)
    {
        ...
        if(isset($this->source[$var]))
        {
            ...
            {
                $this->errors[$var] = $this->validation_rules[$var]['label'] . __(' Es muy corto', 'validate_anaconda');
            }
            elseif(strlen($this->source[$var]) > $max)
            {
                $this->errors[$var] = $this->validation_rules[$var]['label'] . __(' Es muy largo', 'validate_anaconda');
            }
            elseif(!is_string($this->source[$var]))
            {
                $this->errors[$var] = $this->validation_rules[$var]['label'] . __(' No es válido', 'validate_anaconda');
            }
        }
    }
    ...
    private function validateNumeric($var, $min=0, $max=0, $required=false)
    {
        ...
        {
            $this->errors[$var] = $this->validation_rules[$var]['label'] . __(' No es un número Válido', 'validate_anaconda');
        }
    }
    ...
    private function validateUrl($var, $required=false)
    {
        ...
        {
            $this->errors[$var] = $this->validation_rules[$var]['label'] . __(' No es una URL válida', 'validate_anaconda');
        }
    }
    ...
    private function validateEmail($var, $required=false)
    {
        ...
        {
            $this->errors[$var] = $this->validation_rules[$var]['label'] . __(' No es un correo electrónico válido', 'validate_anaconda');
        }
    }
